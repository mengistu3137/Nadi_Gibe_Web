...
const NewsSection = () => {
  ...
  const getFallbackImage = (itemType) => {
    switch (itemType) {
      case "news":
        return defaultImages.news;
      case "event":
        return defaultImages.event;
      case "announcement":
        return defaultImages.announcement;
      default:
        return defaultImages.news;
    }
  };
      ...
      try {
        ...
        if (response.success) {
          ...
          const transformedData = response.data.map((item) => ({
            ...
            date: item.date,
            image: item.image || getFallbackImage(item.type), // Ensure image URL exists
            ...
            <div className="bg-white rounded-2xl overflow-hidden shadow-xl border border-[#F5F4FF]">
              <div className="grid grid-cols-1 lg:grid-cols-2">
                <div className="relative h-64 lg:h-auto">
                  <img
                    src={
                      imageErrors[featuredItem.id]
                        ? defaultImages.featured
                        : featuredItem.image
                    }
